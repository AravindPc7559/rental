...
function District() {
  ...
  const handleClose = () => setOpen(false);
    ...
    const handleChange = async(dis) => {
        ...
        try {
            ...
            axios.post('http://localhost:5000/api/user/searchdistrict',{'place':dis}).then((res)=>{
  ...
  return (
    <div>
        ...
        <Button style={{marginLeft:120}} onClick={handleOpen}>Sort By District</Button>
      <Modal
        open={open}
        onClose={handleClose}
        ...
        <Box sx={style}>
          {
              districtData.map((obj)=>{
                  return(
                    ...
                    <Button onClick={()=>handleChange(`${obj.district}`)} style={{color:'black'}} >{obj.district}</Button>
                    )
                })
            }
        </Box>
